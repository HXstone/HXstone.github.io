...
- **入口**:`projects.html` 第 05 个案例(标题旁带金色「新」标签 `.badge-new`),导航栏 `Games` 指向该案例锚点;首页 Case Studies 第 05 条直达游戏。
...
- **金色是重音不是底色**:全站只有少数几处用金色(主按钮、状态栏的指示与进度、HX OS 的状态灯与日志键、作品行的 hover 规则线与序号);其余一律走中性色
...
- **降级**:JS 未加载时内容完整可读(HX OS 显示真实数字,笔记列表可筛选由 JS 增强);`prefers-reduced-motion` 下所有动画与 View Transition 动画关闭
